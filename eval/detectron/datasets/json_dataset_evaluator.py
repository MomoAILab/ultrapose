...
def _do_body_uv_eval(json_dataset, res_file, output_dir):
    ...
    with open(res_file, 'rb') as f:
        ...
        for idx in range(len(res)):
            # print(np.max(res[idx]['uv'][0,:,:]))
            # print('>>>>', res[idx]['bbox'])
            res[idx]['category_id'] = 1
            res[idx]['score'] = 1.0
            # res[idx]['uv'][0, :, :] -= 1 # old
            if 'uv_shape' in res[idx]:
                res[idx]['bbox'][2] = float(min(res[idx]['bbox'][2], res[idx]['uv_shape'][2]))
                res[idx]['bbox'][3] = float(min(res[idx]['bbox'][3], res[idx]['uv_shape'][1]))
            # print('<<<<', res[idx]['bbox'])
